...
{
    fn can_cast(kind: SyntaxKind) -> bool { kind == BLOCK_COMMENT }
...
{
    fn can_cast(kind: SyntaxKind) -> bool { kind == STR }
...
impl std::fmt::Display for MultilineStr
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result
    {
        std::fmt::Display::fmt(&self.syntax, f)
...
{
    fn can_cast(kind: SyntaxKind) -> bool { kind == MULTILINE_STR }
